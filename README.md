...
Modded [openai/codex](https://github.com/openai/codex) harness that merges an extra **OpenCode Go** provider into the ChatGPT desktop app's model picker — no app copy, no re-signing.
...
```bash
# Quit ChatGPT first (the app is single-instance), then:
./launch-tier2.sh                              # from a checkout
~/.local/share/codex-swapper/launch-tier2.sh  # curl|bash install

# CLI without the app:
codex-mod --help
codex-mod exec -m deepseek-v4-flash "hello"
...
`launch-tier2.sh` forwards `CODEX_CLI_PATH` (modded binary) + `CODEX_HOME=~/.codex` via `launchctl setenv` + `open -a ChatGPT`, so the app shows your real projects/sessions/config.
