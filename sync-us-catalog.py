...
Sources (in this repo under model-catalogs/ or in $CODEX_HOME/model-catalogs/):
  - `opencode-go.json`                : full opencode-go catalog (14 models;
                                        muse-spark-1.2 removed 2026-08-20 after
                                        the gateway dropped it)
  - `opencode-go-us-contributor.json` : the muse-spark-1.2-contributor entry

The two SOURCE files above are not generated by this script; keep their repo
copies (install.sh ships model-catalogs/* into $CODEX_HOME) in sync with
$CODEX_HOME manually.
